...
export const scripts: Script[] = [
  ...
  },
  {
    ...
    {
    id: '3',
    name: 'ניתוח מכירות רבעוני',
    description: 'ניתוח מקיף של נתוני מכירות לרבעון נבחר',
    type: 'ניתוח עסקי',
    tags: ['מכירות', 'ניתוח', 'רבעוני'],
    sections: [
      {
        id: 'time_period',
        title: 'תקופת זמן',
        inputs: [
          { name: 'רבעון', type: 'select', required: true, label: 'רבעון', options: ['Q1', 'Q2', 'Q3', 'Q4'] },
          { name: 'שנה', type: 'number', required: true, label: 'שנה' },
        ],
        validate: (sectionInputs) => {
          const year = Number(sectionInputs['שנה']);
          return (year >= 2000 && year <= new Date().getFullYear()) ? null : 'נא להזין שנה תקינה';
        }
      },
      {
        id: 'analysis_options',
        ...
        inputs: [
          { name: 'השוואה_לרבעון_קודם', type: 'select', required: true, label: 'השוואה לרבעון קודם', options: ['כן', 'לא'] },
          { name: 'פילוח_לפי_מוצר', type: 'select', required: true, label: 'פילוח לפי מוצר', options: ['כן', 'לא'] },
          { name: 'ניתוח_מגמות', type: 'select', required: true, label: 'ניתוח מגמות', options: ['כן', 'לא'] },
        ]
      },
      {
        id: 'output_format',
        title: 'פורמט פלט',
        inputs: [
          { name: 'סוג_דוח', type: 'select', required: true, label: 'סוג דוח', options: ['PDF', 'Excel', 'PowerPoint'] },
          { name: 'כלול_גרפים', type: 'select', required: true, label: 'כלול גרפים', options: ['כן', 'לא'] },
        ]
      }
    ]
  },
